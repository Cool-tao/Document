...
class Sample05 {
    ...
    private var getMessageListJob: Job? = null
    fun getMessageList() {
        "A主线程 ${BaseUtil.isUIThread()}".logI()
        getMessageListJob = launch(UI) {
            "B0主线程 ${BaseUtil.isUIThread()}".logI()
            val userId = withContext(CommonPool) { getUserId(3000L) }
            "B1主线程 ${BaseUtil.isUIThread()}".logI()
            val emailMessage = async(CommonPool) { getEmailMessage(4000L, userId) }
            val smsMessage = async(CommonPool) { getSmsMessage(2000L, userId) }
            listOf(emailMessage, smsMessage).forEach {
                it.join()
            }
            "B2主线程 ${BaseUtil.isUIThread()}".logI()
            onGetMessageList("${emailMessage.await()}  ， ${smsMessage.await()}")
        }
        "C主线程 ${BaseUtil.isUIThread()}".logI()
    }

    fun cancelGetMessageList() {
        val cancel = getMessageListJob?.cancel()
        cancel?.logE()
    }
